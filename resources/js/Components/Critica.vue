...
// Props:
// 'critica': con sus datos necesarios;
const props = defineProps(['critica', 'indice']);
...
<template>
    ...
    <div class="w-full text-right mb-6">
        ...
        <Link v-if="$page.props.auth.user" as="button" method="post"
              :href="route('darLike')"
              :data="{ usuario_id: $page.props.auth.user['id'], critica_id: critica.id }"
              preserveScroll>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                 :fill=" $page.props.criticas.data[indice].likes.map(critica => critica.id).includes($page.props.auth.user.id) ? 'black' : 'white'"
